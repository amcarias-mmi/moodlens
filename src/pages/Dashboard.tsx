...
import { BookOpen, Flame, Pencil, Plus, TrendingUp } from 'lucide-react';
      ...
      <div className='space-y-8'>
        ...
        >
          ...
          {streak > 0 && (
            ...
            >
              <Badge className="gap-1.5 px-3 py-1.5 bg-amber-50 text-amber-700 border-amber-200 text-sm font-semibold rounded-full hover:bg-amber-100 dark:hover:bg-primary/80 transition-colors">
        ...
        >
          {isLoading ? (
            ...
            /* ── Already logged ── */
            <TodayCard entry={todayEntry} onEdit={() => setModalOpen(true)} />
          ) : (
            /* ── Not yet logged ── */
...
function StatCard({
  ...
  return (
    <div className='bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-700/50 p-4 shadow-sm'>
      ...
      <p className='font-display text-3xl font-medium text-stone-900 dark:text-stone-50'>
        {value}
      </p>
    </div>
  );
